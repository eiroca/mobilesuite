 ...
 * href="#getEventType()">getEventType()</a> method. Initially, the parser is
 ...
 * href="http://xmlpull.org/v1/doc/properties.html#xmldecl-version">http://xmlpull.org/v1/doc/properties.html#xmldecl-version</a>&quot;)
 * returns String ("1.0") or null if XMLDecl was not read or if property is not
 * supported
 * <li><b>standalone</b>: getProperty(&quot;<a
 * href="http://xmlpull.org/v1/doc/features.html#xmldecl-standalone">http://xmlpull.org/v1/doc/features.html#xmldecl-standalone</a>&quot;)
...
public interface XmlPullParser {
   ...
   * href="#nextToken()">nextToken()</a> when a start tag was read. The name of
   ...
   * the CDATA section is available by callling getText().
   * @see #nextToken
   * @see #getText
   */
  int CDSECT = 5;
   ...
   * will accumulate the replacement text and other text events to a single TEXT
   * event.
   * @see #nextToken
   * @see #getText
   */
   ...
   * via <a href="#nextToken()">nextToken()</a> only; calls to next() will skip
   * comments automatically. The content of the comment can be accessed using
   * the getText() method.
   * @see #nextToken
   * @see #getText
   */
   ...
   * from <a href="#nextToken()">nextToken()</a> only. The unparsed text inside
   * the doctype is available via the getText() method.
   * @see #nextToken
   * @see #getText
   */
  int DOCDECL = 10;
   ...
   * START_TAG or TEXT to to a string. For example, the value of
   ...
   * dangerous since malicous applications may alter the array, although it is
   * final, due to limitations of the Java language.
   */
  String[] TYPES = {
   ...
   * set to false, the DOCDECL event type is reported by nextToken() and ignored
   * by next(). If this featue is activated, then the document declaration must
   ...
   * is true and both, the internal and external document type declaration will
   * be processed.
   * <p>
   ...
   * called before the first call to next or nextToken. Otherwise, an exception
   * is thrown.
   * <p>
   ...
   * href="http://xmlpull.org/v1/doc/features.html#detect-encoding">http://xmlpull.org/v1/doc/features.html#detect-encoding</a>
   ...
   * setInput(InputStream, inputEncoding) was called with an inputEncoding value
   * other than null, this value must be returned from this method. Otherwise,
   * if inputEncoding is null and the parser suppports the encoding detection
   * feature (http://xmlpull.org/v1/doc/features.html#detect-encoding), it must
   ...
   * href="http://www.w3.org/TR/REC-xml#intern-replacement">XML 1.0 Section 4.5
   ...
   * XMLPULL that will work in J2ME environments. Though these implementations
   * may not be able to process the document type declaration, they still can
   * work with known DTDs by using this function.
   * <p>
   * <b>Please notes:</b> The given value is used literally as replacement text
   * and it corresponds to declaring entity in DTD that has all special
   * characters escaped: left angle bracket is replaced with &amp;lt;, ampersnad
   * with &amp;amp; and so on.
   * <p>
   ...
   * depth is 0. The depth is incremented by 1 when a start tag is reached. The
   * depth is decremented AFTER the end tag event was observed.
   *
   * <pre>
   *      &lt;!-- outside --&gt;     0
   *      &lt;root&gt;                  1
   ...
   * not know the current column number or can not determine it, -1 is returned
   * (e.g. for WBXML).
   ...
   * always return the same value as getText(), except for ENTITY_REF. In the
   * case of ENTITY ref, getText() returns the replacement text and this method
   * returns the actual input buffer containing the entity name. If getText()
   * returns null, this method returns null as well and the values returned in
   * the holder array MUST be -1 (both start and length).
   * @see #getText
   * @see #next
   * @see #nextToken
   * @param holderForStartAndLength Must hold an 2-element int array into which
   *            the start offset and length values will be written.
   ...
   * are enabled and the prefix is not null, you will need to add the prefix and
   * a colon to localName..
   */
  String getName();
   ...
   * from 0). Returns an empty string ("") if namespaces are not enabled or the
   * attribute has no namespace. Throws an IndexOutOfBoundsException if the
   * index is out of range or the current event type is not START_TAG.
   * <p>
   ...
   * Throws an IndexOutOfBoundsException if the index is out of range or current
   ...
   * not passed, an exception is thrown. The exception text indicates the parser
  ...
  /**
   * Call next() and return event if it is START_TAG or END_TAG otherwise throw
